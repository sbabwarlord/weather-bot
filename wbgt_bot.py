...
def load_previous_uv():
    ...
    return data.get("uv_value")


def save_current_uv(uv_value):
    with open(STATE_FILE, "w") as f:
        json.dump({"uv_value": uv_value}, f)


def should_broadcast(previous_uv, current_uv):
    ...
    if previous_uv in range(0, 3) and current_uv in range(0, 3):
        return False
...
def fetch_wbgt():
    ...
    station_data = next(
        (r for r in readings if r["station"]["id"] == TARGET_STATION),
        None
    )
...
async def main():
    ...
    uv_value = fetch_uv()
    previous_uv = load_previous_uv()

    if not should_broadcast(previous_uv, uv_value):
        save_current_uv(uv_value)
        return
